...
function handleSendMessage(e) {
    ...
    // Fase 3: conversas vinculadas a um pedido de ajuda (js/doacoes.js,
    // openPedidoChat) são reais — a mensagem vai para o Firestore e a
    // própria sincronização em tempo real (onSnapshot) atualiza a tela.
    // As conversas de demonstração (Suporte, ONGs de exemplo etc.) continuam
    // funcionando exatamente como antes, com resposta automática simulada.
    if (chat?.firestore && typeof chat.send === "function") {
        chat.send(text);
        return;
    }
...
// Fase 3: usados por js/doacoes.js para repintar o chat quando uma nova
// mensagem chega em tempo real (onSnapshot) numa conversa vinculada a um pedido.
