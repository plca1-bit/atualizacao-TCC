...
const initialUsers = [
    ...
    { id: "u-common1", email: "maria@email.com", password: "maria123", name: "Maria do Carmo", role: "person", cpf: "222.333.444-55", phone: "(11) 98765-4321", address: "Rua das Rosas, 12, Bairro Alto, São Paulo - SP" },
    { id: "u-common2", email: "roberto@email.com", password: "roberto123", name: "Roberto Souza", role: "person", cpf: "777.888.999-00", phone: "(21) 99999-8888", address: "Rua General Osório, 145, Ipanema, Rio de Janeiro - RJ" }
];
...
// Compatibilidade com os módulos de Doações, Administração e Brechó.
window.appState = {
...
document.addEventListener("includesLoaded", () => {
    ...
    if (clearNotifBtn) {
        clearNotifBtn.addEventListener("click", () => {
            ...
            const remaining = state.getNotifications().filter((notification) => notification.recipient_id && notification.recipient_id !== currentUserId);
            state.setNotifications(remaining);
            renderNotifications();
    ...
    function addNotification(text) {
        const currentUserId = state.currentUser?.id;
        const notifs = state.getNotifications().filter((notification) => !notification.recipient_id || notification.recipient_id === currentUserId);
    ...
    function renderNotifications() {
        ...
        notifs.forEach(n => {
            ...
            li.addEventListener("click", () => {
                ...
                const matching = allNotifications.find((notification) => notification.id === n.id);
                if (matching) matching.read = true;
                state.setNotifications(allNotifications);
                renderNotifications();
            });
    ...
    function applySelectedRole(role) {
        const institutionFields = [
            document.getElementById("field-ong-doc-group"),
        ...
        if (isOng) {
            ...
            if (hint) hint.innerText = "Pessoa: doe itens, solicite ajuda, compre no brechó e acompanhe seu histórico, tudo com o mesmo cadastro.";
        }
    }
    ...
    (function configurarValidacaoCadastro() {
        ...
        if (campoDoc) {
            ...
            campoDoc.addEventListener("blur", () => {
                ...
                const valido = papelAtual() === "ong" ? V.validarCNPJ(campoDoc.value) : V.validarCPF(campoDoc.value);
                V.mostrarErroCampo(campoDoc, valido ? "" : (papelAtual() === "ong" ? "CNPJ inválido. Confira os números digitados." : "CPF inválido. Confira os números digitados."));
            });
        }
    ...
    const fileInput = document.getElementById("reg-wizard-ong-doc");
    ...
    function performLogin(user) {
        ...
        if (user.role === "admin" || user.role === "ong") {
        ...
        if (typeof renderRequests === "function") renderRequests();
        if (typeof renderBrechoSellerNotifications === "function") renderBrechoSellerNotifications();
    ...
    function updateUserAuthUI() {
        ...
        const canManageLogistics = user && (user.role === "admin" || user.role === "ong" || user.role === "institution");
        document.querySelectorAll(".admin-only").forEach((element) => element.classList.toggle("hidden", !canManageLogistics));
    }
